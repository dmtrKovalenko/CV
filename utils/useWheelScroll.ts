...
import { useDomEvent, MotionValue, PanInfo } from "framer-motion";
...
const debouncedSpringTo = debounce(springTo, 100) as any;
...
export function useWheelScroll({
  ...
  const processScroll = (deltaY: number) => {
    ...
    setTimeout(() => onWheelCallback(), 250)
  };

  const handleWheel = (event: WheelEvent) => {
    event.preventDefault();

    processScroll(event.deltaY)
  };

  // @ts-ignore
  useDomEvent(ref, "wheel", isActive && handleWheel, { passive: false });
